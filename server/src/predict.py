...
def predict():
    ...
    actual_dates = dataset.index[-358:].tolist() 
    actual = dataset.iloc[-358:].values.astype('float64').reshape(1, 358)
    ...
    try:
        if param1 == 'BTC':
            ...
            n,m = 0.8, 0.1
            model1 = create_model(dataset, n, m)
            forecast = model1.forecast(steps=7)
            print(forecast)
            ...
            future_predictions = model(param1, 0, api_key)
            future_predictions = future_predictions.astype('float32').reshape(1, 7).tolist()
            scaler = joblib.load(f'./data/{param1}Scaler.pkl')
            ...
            future_predictions = model(param1, 0.0246, api_key)
            future_predictions = future_predictions.reshape(1, 7).tolist()
            print(future_predictions)
        else:
        ...
        next_predicted_value = future_predictions
